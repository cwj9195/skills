...
> 每个有状态流转的模块必须包含本节。纯静态展示页可标注"本模块无状态流转"跳过。
...
> 每个涉及表单/表格的模块必须列出。字段数量 > 20 时按业务分组。
> 字段必须与 PRD 逐一对应，不允许遗漏。

#### 分组一：{分组名}（如 SPU 公共信息）

| 字段名 | key | 控件类型 | 必填 | 校验规则 | 枚举值/数据源 |
| --- | --- | --- | --- | --- | --- |
| TODO | `TODO` | 文本/下拉/选择器 | 是/否 | TODO | TODO |

#### 分组二：{分组名}（如 价格信息）

| 字段名 | key | 控件类型 | 必填 | 校验规则 | 枚举值/数据源 |
| --- | --- | --- | --- | --- | --- |
| TODO | `TODO` | 文本 | 否 | 小数N位，≤M位正数 | — |

### 4.5 组件复用/沿用

| 组件 | 来源 | 职责 | props | emit | 使用场景 | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `<TODO />` | TODO | TODO | TODO | TODO | TODO | `path:line` |

#### 4.5.1 新建组件

| 中文组件名 | 文件路径 | 职责 | Props | Emit |
| --- | --- | --- | --- | --- |
| `<TODO />` | `pages/xxx/components/Todo.tsx` | TODO | TODO | TODO |
...
| 类型 | 操作 | 路径 |
| --- | --- | --- |
| 页面 | 修改 | `apps/xxx/pages/yyy/index.tsx` — TODO |
| 组件 | 新建 | `apps/xxx/pages/yyy/components/Todo.tsx` |
| API | 修改 | `apps/xxx/request/record/goods/yyy/index.ts` — 新增接口 |
| 类型 | 新建 | `apps/xxx/pages/yyy/types.ts` |
