...
## 7. session-bridge MCP 与跨工具会话续传经验

> 来源：2026-06-18 实现 session-bridge MCP 全过程。适用范围：跨工具 MCP 开发、cc-switch 维护、会话读取。状态：有效。最后确认时间：2026-06-18。
...
### 7.2 现成工具调研结论（都是「导出→开新会话 resume」，做不到回原会话增量补）
...
### 7.3 codex/claude 会话存储格式（session-bridge 解析依据）
...
### 7.6 cc-switch 的 MCP 管理（源信息 = cc-switch.db）
