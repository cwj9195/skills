...
> **定位**：本文档是实现 Agent 的唯一执行入口，承载任务拆解、门禁命令、回归检查与停止条件。默认不再生成独立 `tasks.md` 或 `ci-gate.md`。
...
如果用户显式提供旧英文任务包路径，可作为普通输入读取；如本轮需要更新任务包，写回中文路径 `设计/编码任务.md`，不得同时维护两份任务事实源。
...
| 顺序 | 文件                | 用途                                                                | 必读 |
| ---- | ------------------- | ------------------------------------------------------------------- | ---- |
| 1    | `代码参考.md`       | 代码模式参考（最先读，理解项目怎么写的）                            | 是   |
| 2    | `前端设计.md`       | 模块实现方案（引用 代码参考 章节）                                  | 是   |
| 3    | `需求说明.md`       | 需求范围、字段、权限、验收 + 澄清记录                               | 是   |
| 4    | `../../接口契约.md` | 默认接口契约总账；任务涉及接口时读取对应接口 ID，无接口任务确认“无” | 是   |
| 5    | `模块索引.md#M{N}`  | Manifest 单一入口源；路径或模块状态不清时回溯当前模块记录           | 否   |

> **v2.3 治理收敛**：`facts.md`、`tasks.md`、`ci-gate.md`、`agent-prompts.md`、`fact-set.yaml` 默认不生成。
> 需求事实进入 需求说明；代码事实进入 代码参考；接口契约进入 接口契约；实现方案进入 前端设计；任务治理、门禁命令、回归检查和停止条件进入本文件。
...
| --------- | ---- | -------- | -------- | ------------------ | ------------------ | -------------------------------------- | ----------- | -------- | -------- | -------- | -------- |
| [T-M{N}-01](#t-m{N}-01) | TODO | TODO | TODO | [REQ-M{N}-001](需求说明.md#req-m{N}-001) | [代码参考 §1](代码参考.md#code-ref-1) | [API-M{N}-TODO](../../接口契约.md#api-m{N}-todo) / 无 | TODO | TODO | TODO | TODO | TODO |
...
| ---- | ---- |
| 任务 ID | T-M{N}-01 |
| 需求引用 | [REQ-M{N}-001](需求说明.md#req-m{N}-001) |
| 代码引用 | [代码参考 §1](代码参考.md#code-ref-1) |
| API 引用 | [API-M{N}-TODO](../../接口契约.md#api-m{N}-todo) / 无 |
| 停止条件 | TODO |
...
- `目标`：一句话描述本任务完成后的代码状态。
- `涉及文件`：精确到文件路径；新增文件写完整路径。
- `需求引用`：引用 需求说明.md 中真实存在的 ID 或章节，使用 `[REQ-M{N}-001](需求说明.md#req-m{N}-001)` 形式。
- `代码引用`：引用 `代码参考.md` 中真实存在的显式锚点，例如 `[代码参考 §1](代码参考.md#code-ref-1)`，禁止使用占位或不存在的假锚点。
- `API 引用`：任务涉及接口时引用 `接口契约.md` 中真实存在的接口 ID，使用 `[API-M{N}-TODO](../../接口契约.md#api-m{N}-todo)` 形式；无接口任务写“无”。
