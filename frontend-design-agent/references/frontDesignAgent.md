...
- `implement` 是约束最强的阶段，只能按 `implementation-tasks.md` 的任务 ID 执行。
...
输入材料
  ├─ PRD / MasterGo / 原型 / 截图
  ├─ module-index.md + 模块文件夹
  └─ implementation-tasks.md
        ↓
mode_select
        ├─ module-split：生成 module-index.md + 模块文件夹
        ├─ module-design：生成 5 份核心设计产物
        └─ implement：按 implementation-tasks.md 执行代码改动
```
...
| 任务越界实现                           | `implement` 只按 `implementation-tasks.md` 的任务 ID 执行                 |
